//+------------------------------------------------------------------+
//| MomenCandleStreak_Ultimate_V4.1.mq5                       |
//| Combined: v4.1 (v3.1 Multi-Sym Engine/SL Cap/Session) + vD1.3 (3-Layer)|
//+------------------------------------------------------------------+
#property copyright "Custom EA - Combined Hybrid Strategy v4.1"
...
input double Layer1_LotSize            = 0.05;   // Lot khusus Layer 1 (jika UseStaticLot = true)
...
input double Layer2_LotSize            = 0.05;   // Lot khusus Layer 2 (jika UseStaticLot = true)
...
input double Layer3_LotSize            = 0.05;   // Lot khusus Layer 3 (jika UseStaticLot = true)
...
input int    LimitExpireBars           = 3;      // Masa berlaku Pending Order Layer 2 & 3 (dalam jumlah Bar)

input group "=== 6. Filter Ukuran Streak (Min & Max Points) ==="
input bool   UseStreakSizeFilter       = true;   // Aktifkan Filter Ukuran Candle Streak
input double MinStreakPoints           = 300;    // Batas Minimal Total Points Streak
input double MaxStreakPoints           = 3000;   // Batas Maksimal Total Points Streak

input group "=== 7. Lot Management & Risk ==="
input bool   UseStaticLot              = true;   // true = Static Lot, false = % Risk Balance
input double StaticLotSize             = 0.05;   // Base Static Lot Size (fallback global)
input double RiskPercentPerTrade       = 1.0;    // % Risk Balance per trade jika UseStaticLot = false

input group "=== 8. Target Stop Loss (Dynamic vs Static SL) ==="
input bool   UseStaticSL               = true;   // true = Pakai StaticSLPoints, false = Dynamic SL (Open Candle / Low-High)
...
input double MaxSLPoints               = 1000;   // BATAS MAKSIMAL Dynamic SL (0 = Tanpa Batas / Capped)
...
int OnInit()
  {
   ...
   Print("EA Hybrid v3.5 Initialized. Total Active Symbol/TF States: ", idx);
...
bool ExecuteLayeredTrade(string sym, ENUM_TIMEFRAMES tf, ENUM_ORDER_TYPE type, datetime barTime)
  {
   ...
   // Check Filter Ukuran Streak Points (Batas Min / Max)
   double streakDist = MathAbs(iClose(sym, tf, 1) - iOpen(sym, tf, StreakCount)) / point;
   ...
   // Perhitungan SL Induk (Base SL Level)
   ...
   if(UseStaticSL)
     ...
     {
      double slOpenCandle = iOpen(sym, tf, firstCandleIdx);
      double rawSL = (type == ORDER_TYPE_BUY) ? (slOpenCandle - SL_BufferPoints*point) 
                                              : (slOpenCandle + SL_BufferPoints*point);
      slBase = rawSL;

      // Perhitungan Capping Max SL Points
   ...
   // --- 1. EKSEKUSI LAYER 1 (Market Instant Order - v3.1) ---
   if(UseLayer1)
     {
      double slL1 = slBase;
      ...
      if(R1 > 0)
        {
         ...
         double lotL1 = UseStaticLot ? NormalizeLot(sym, Layer1_LotSize) : CalculateDynamicLot(sym, R1/point);
   ...
   // --- 2. EKSEKUSI LAYER 2 (Pending Limit Order Retrace 40% - vD1.3) ---
   if(UseLayer2)
     {
      ...
      if(type == ORDER_TYPE_BUY)
        {
         ...
         if(R2 > 0 && limitPriceL2 <= (ask - stopLevel)) 
           {
            double tpL2 = limitPriceL2 + (R2 * Layer2_RiskRewardRatio);
            double lotL2 = UseStaticLot ? NormalizeLot(sym, Layer2_LotSize) : CalculateDynamicLot(sym, R2/point);
   ...
   // --- 3. EKSEKUSI LAYER 3 (Pending Limit Order Retrace 70% - vD1.3) ---
   if(UseLayer3)
     {
      ...
      if(type == ORDER_TYPE_BUY)
        {
         ...
         if(R3 > 0 && limitPriceL3 <= (ask - stopLevel)) 
           {
            double tpL3 = limitPriceL3 + (R3 * Layer3_RiskRewardRatio);
            double lotL3 = UseStaticLot ? NormalizeLot(sym, Layer3_LotSize) : CalculateDynamicLot(sym, R3/point);
            datetime expr = barTime + (LimitExpireBars * PeriodSeconds(tf));
   ...
   switch(dt.day_of_week)
     {
      ...
      default: return false; // Sabtu/Minggu selalu OFF
     }
  }

...
// v1.4: base dari kode user (StreakCount=2, RR awalnya 1.0) dengan perubahan:
// 1. SL_BufferPoints = 200
// 2. MaxSpreadPoints = 500
// 3. Session filter DIHAPUS total (input + fungsi + pemanggilan)
// 4. RiskRewardRatio default 1.5 (tetap bisa diubah manual, TIDAK di-hardcode)
// 5. Breakeven: SL pindah ke entry saat profit capai BreakevenTriggerRR x R (default 1:1)
#property strict
...
input double BreakevenTriggerRR  = 1.0;   // pindahkan SL ke BEP saat floating profit mencapai X * R (default 1:1)
...
void OnTick()
  {
   ManageBreakeven(); // dievaluasi tiap tick agar responsif terhadap floating profit
...
void ExecuteTrade(string sym, ENUM_TIMEFRAMES tf, ENUM_ORDER_TYPE type)
  {
   ...
   string cmt = "S3v5|" + DoubleToString(R, _Digits); // simpan R untuk logika BEP
...
//+------------------------------------------------------------------+
//| Breakeven: pindahkan SL ke harga entry saat floating profit      |
//| mencapai BreakevenTriggerRR x R (default 1:1). Hanya sekali,     |
//| dan tidak pernah memindahkan SL menjauh dari entry.               |
//+------------------------------------------------------------------+
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
     {
      ...
      if(profitR < BreakevenTriggerRR) continue;

      if(type == POSITION_TYPE_BUY)
        {
         if(currentSL < entry) // SL belum di BEP, dan hanya boleh maju
            trade.PositionModify(ticket, entry, PositionGetDouble(POSITION_TP));
        }
      else
        {
         if(currentSL == 0 || currentSL > entry)
            trade.PositionModify(ticket, entry, PositionGetDouble(POSITION_TP));
        }
     }
  }
